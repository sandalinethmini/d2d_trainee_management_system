...
class UserModel extends  CI_Model {
    ...
    public function updateRecord($userId,$fullName,$pagesList,$code)  //OK
	{
		$this->db->trans_begin();
			$data = array('branch_name' => $fullName/*'system_user_area_id'=>$cmbLocationCode*/);
			$this->db->where('branch_id', $userId);
	...
	public function saveRecord($userName,$fullName,$pagesList,$code) // OK
	{
		$this->db->trans_begin();
		
		$data = array('branch_email' => $userName,'branch_password' => md5($code),'branch_name' => $fullName, 'branch_status' => 1, 'branch_code'=>$code );
	...
	public function changeStatus($userId,$userStatus) //OK
	{
		$this->db->trans_begin();

		$data = array('branch_status' => $userStatus);
		$this->db->where('branch_id', $userId);
		$this->db->update('system_users', $data); 
...
$this->db->close();	
	...
	public function changePassword($userId,$userName) //Ok
	{
		$this->db->trans_begin();

		$data = array('branch_password' => md5($userName));
		$this->db->where('branch_id', $userId);
		$this->db->update('system_users', $data); 
	...
	public function editRecord($userId)  // OK
	{		
		$this->db->select('branch_id, branch_email, branch_name,branch_code');
		$this->db->from('system_users');
		//$this->db->join('area','area_id = system_user_area_id','inner');
		$this->db->where('branch_id', $userId);
