...
class SiteModel extends CI_Model {
	...
	var $order = array('site_branch_id' =>
 'asc'); // default order 
	...
	{
		$this->db->select('bandwidth_type_id,bandwidth_type');
		$this->db->from('bandwidth_type_details');
		$this->db->order_by('bandwidth_type', 'asc');
		$query=$this->db->get();
		
		$BandwidthType = array();
		
		$BandwidthType = array('' => 'BANDWIDTH TYPE');
		foreach ($query->result_array() as $row) 
		{
			$BandwidthType[$row['bandwidth_type_id']] = $row['bandwidth_type'];
		}	
	...
	public function save_record($branch,$category,$officer,$circuitid,$accno,$provider,$iprange,$connection,$startdate,$cmbBandwidthType,$cmbCurrentBandwidth)
	{
		$this->db->trans_begin();
		
		$data = array('site_branch_id' => $branch, 'site_category' => $category,'site_officer_name'=> $officer,'site_circuit_id' => $circuitid,'site_account_no' => $accno,'site_provider' => $provider ,'site_iprange' => $iprange,'site_connection_type' => $connection,'site_connection_started_date' => $startdate,'site_bandwidth_type' => $cmbBandwidthType ,'site_current_bandwidth' => $cmbCurrentBandwidth);
		...
		if ($this->db->trans_status() === FALSE)
		{
				...
				header( 'Location: '.base_url("index.php/application/Site") ) ; 
		}
		else
		{
	...
	public function editRecord($site_id)
		{		
			$this->db->select('site_id,branch_code,branch_description,category_id,officer_id,provider_id,connection_id,site_circuit_id,site_account_no,provider_description,site_iprange,site_connection_started_date,site_bandwidth_type,site_current_bandwidth');
			$this->db->join('provider_details','provider_id = site_provider','inner');
	...
	public function update_record($branch, $category, $officer, $circuitid, $accno, $provider, $iprange, $connection,$startdate,$cmbBandwidthType,$cmbCurrentBandwidth,$site_id)
	{
		$this->db->trans_begin();
	...
	public function update_LinkDetails($NewStartDate,$NewBandwidthType,$NewBandwidth,$site_id)
	{
		$this->db->trans_begin();
		
		//$data = array('site_bandwidth_type' => $BandwidthType, 'site_current_bandwidth' => $CurrentBandwidth,'site_connection_started_date'=> $startdate,'site_start_date' => $NewStartDate, 'site_bandwidth_type' => $NewBandwidthType,'site_current_bandwidth'=> $NewBandwidth);
		
		
		$this->db->select('site_id,site_bandwidth_type,site_current_bandwidth,site_connection_started_date');
		$this->db->from('site_details');
		$this->db->where('site_id', $site_id); 
		$result = $this->db->get()->row();
			
		$data = array('history_bandwidth_close_date' => $NewStartDate);

		$this->db->where('history_bandwidth_site_id', $site_id);
		$this->db->where('history_bandwidth_type',$result->site_bandwidth_type);
		$this->db->where('history_bandwidth', $result->site_current_bandwidth);
		$this->db->where('history_bandwidth_start_date',$result->site_connection_started_date);
		
		$this->db->update('history_bandwidth', $data);
		
		
		$data = array('site_bandwidth_type' => $NewBandwidthType,'site_current_bandwidth' => $NewBandwidth,'site_connection_started_date' => $NewStartDate);
		
		$this->db->where('site_id', $site_id);
		$this->db->update('site_details', $data);
		
		
		$data = array('history_bandwidth_type' => $NewBandwidthType ,'history_bandwidth' => $NewBandwidth,'history_bandwidth_start_date' => $NewStartDate,'history_bandwidth_site_id'=>$site_id);	
		
		$this->db->insert('history_bandwidth', $data); 
			
		if ($this->db->trans_status() === FALSE)
		{
				$this->db->trans_rollback();
				$this->session->set_flashdata('fail', 'Data Not Updated. Try Again');
				header( 'Location: '.base_url("index.php/application/Site") ) ; 
		}
		else
		{
